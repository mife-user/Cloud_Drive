...
func (r *userRepo) Logon(ctx context.Context, user *domain.User) error {
	...
	if err == nil {
		if cache.IsNullValue(userjsonOut) {
			logger.Debug("登录用户失败"+errorer.ErrUserNotExist, logger.S("user_name", existingUser.UserName))
			return errorer.New(errorer.ErrUserNotExist)
		}
		if err = exc.ExcJSONToFile(userjsonOut, existingUser); err != nil {
			logger.Error("从缓存中解析用户信息失败", logger.S("user_name", existingUser.UserName), logger.C(err))
			return err
		}
	} else {
		// 缓存中不存在用户，从数据库查询
		if err = r.db.Where("user_name = ?", existingUser.UserName).First(existingUser).Error; err != nil {
			if err = cache.CacheNullValue(ctx, r.rd, key); err != nil {
				logger.Warn("缓存空值失败", logger.C(err))
			}
			logger.Error("登录用户失败", logger.S("user_name", existingUser.UserName), logger.C(err))
			return err
		}
		// 使用带随机偏移的缓存策略
		userjsonIn, err := exc.ExcFileToJSON(existingUser)
		if err != nil {
			logger.Error("缓存用户信息失败", logger.S("user_name", existingUser.UserName), logger.C(err))
			// 缓存失败不影响登录结果
		}
	...
	if !utils.CheckPasswordHash(user.PassWord, existingUser.PassWord) {
		logger.Debug("登录用户失败"+errorer.ErrPasswordError, logger.S("user_name", existingUser.UserName))
