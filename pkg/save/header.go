...
// SaveHeader 保存用户头像
func SaveHeader(ctx context.Context, fileHeader *multipart.FileHeader, username string, rating int) (string, error) {
	var err error
	//获取后缀
	...
	defer file.Close()
	// 保存文件
	headerPath := fmt.Sprintf("./header/%v", username)
	// 检查目录是否存在，不存在则创建
